...
use ndarray::prelude::*;
use ndarray::{arr2, Array, ArrayBase, Axis, Dim, OwnedRepr};
use rand;
...
fn contingency_table(
    clusters1: &Vec<HashSet<u8>>,
    clusters2: &Vec<HashSet<u8>>,
    ...
    let cont_table_vec: Vec<u8> = product
...
fn cluster_size_sequence_sqsum(clusters: &Vec<HashSet<u8>>) -> u8 {
    let cluster1_size_seq: Vec<u8> = clusters.iter().map(|v| v.len() as u8).collect();
...
fn main() {
    ...
    let clusters1: Vec<HashSet<u8>> = clusters1.iter().map(|v| hashset(&v)).collect();
    let clusters2 = vec![vec![0u8, 4, 7], vec![1u8, 2, 3, 6], vec![8u8, 5]];
    let clusters2: Vec<HashSet<u8>> = clusters2.iter().map(|v| hashset(&v)).collect();
